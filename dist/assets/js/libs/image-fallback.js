...
function initImageFallback() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        images.forEach(img => {
            // Chỉ xử lý khi đã load xong nhưng hỏng (naturalHeight === 0).
            // Không dùng !img.complete — ảnh đang tải sẽ bị gán src undefined và không hiện.
...
function handleImageError(img, fallbackUrl = null) {
    ...
    // URL ảnh thay thế
    const newSrc = fallbackUrl || img.dataset.fallback;
    if (!newSrc) {
        img.style.opacity = '0';
        img.classList.add('error');
        return;
