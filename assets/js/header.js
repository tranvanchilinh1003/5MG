...
// ===========================================
// Nav active theo trang (home-hero-nav-list)
// ===========================================
(function syncHeaderNavActive() {
    function pathToNavKey(pathname) {
        ...
        if (seg === 'index' || seg === '') return 'index';
        return seg;
    }

    function hrefToNavKey(href) {
        if (!href || href === '#' || href.startsWith('#') || href.startsWith('javascript:')) return null;
        try {
            var u = new URL(href, window.location.href);
            return pathToNavKey(u.pathname);
        } catch (e) {
            var s = href.split('#')[0].split('?')[0].replace(/^\.\//, '');
            var last = s.split('/').filter(Boolean).pop() || '';
            if (!last) return 'index';
            last = last.replace(/\.html$/i, '').toLowerCase();
            return last === 'index' || last === '' ? 'index' : last;
        }
    }

    function run() {
        var current = pathToNavKey(window.location.pathname);
        document.querySelectorAll('.home-hero-nav-list a[href]').forEach(function (a) {
            a.classList.remove('home-hero-nav-is-active');
            a.classList.add('home-hero-nav-is-muted');
            a.removeAttribute('aria-current');
            var key = hrefToNavKey(a.getAttribute('href'));
            if (key !== null && key === current) {
                a.classList.remove('home-hero-nav-is-muted');
                a.classList.add('home-hero-nav-is-active');
